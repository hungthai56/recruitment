...
export default function CreatePopup(props) {
    ...
    const { listcandidateinterview, isLoading, paging, listdetail } = useSelector(
        (state) => state.Candidateinterview,
    );
   useEffect(() => {
        if (listdetail) {
            methods.setValue("RecruitmentId", listdetail ? listdetail.RecruitmentId : ""),
                methods.setValue("Interviewer", listdetail ? listdetail.Interviewer : ""),
                methods.setValue("typeinterview", listdetail ? listdetail.Type : ""),
                methods.setValue("DateStart", listdetail ? listdetail.TimeStart : ""),
                methods.setValue("TimeStart", listdetail ? listdetail.TimeStart : ""),
                methods.setValue("InterviewTime", listdetail ? listdetail.InterviewTime : ""),
                methods.setValue("BranchId", listdetail ? listdetail.BranchId : ""),
                methods.setValue("Note", listdetail ? listdetail.Note : ""),

                methods.setValue("SendMailCandidate", listdetail.SendMailCandidate == 1 ? true : false),
                methods.setValue("SendMailInterviewer", listdetail.SendMailInterviewer == 1 ? true : false)
    ...
    useEffect(()=>{
        dispatch({
            ...
            payload: listdetail?.RecruitmentId,

        })

    },[listcandidateinterview?.RecruitmentId ])
    ...
    const [listCondidatesid, setlistCondidatesid] = useState(null);
    const [listCondidatesidnew, setlistCondidatesidnew] = useState(null);
    
    useEffect(() => {
        const listCondidatestotal = entries(listCondidates).map(([key, value]) => ({
            value: parseInt(value.CandidateId),
            label: value.CandidateName,
        }));
        setlistCondidatesid(listCondidatestotal);
        const listCondidatesnew = entries(listCondidates).map(([key, value]) => ({
            CandidateId: parseInt(value.CandidateId),
            Email: value.Email,
            Phone: value.Phone,
            TimeStartInterview: value.TimeStartInterview,
        ...
        listCondidatesnew.forEach((item) => {
            prepend(item); // Đưa từng đối tượng vào mảng 'Data'
        });
    }, [listCondidates, prepend]);
